            ...
            <div class="card card-custom">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="card-title">
                        <h3 class="card-label">Danh sách chứng chỉ
                            <span class="d-block text-muted pt-2 font-size-sm">Sorting &amp; pagination remote
                                datasource</span>
                        </h3>
                    </div>
                    <div class="card-toolbar">
                        <a href="{{ route('admin.certificate.create') }}" class="btn btn-primary font-weight-bolder">
                        ...
                        <table class="table table-separate table-head-custom table-checkable">
                            <thead>
                                <tr>
                                    <th>Tiêu đề</th>
                                    <th>Nội dung</th>
                                    <th>Thuộc khoá học</th>
                                    
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($certificate as $db)
                                    <tr>
                                        <td>{{ $db->title }}</td>
                                        <td>{{ $db->description }}</td>
                                        <td><a href="">{{ $db->course_id }}</a></td>
                                        {{-- <td>
                                            <a class="btn btn-light btn-sm" onclick="return confirm('Bạn có chắc muốn xóa')"
                                                href="{{ route('admin.certificate.delete', ['id' => $db->id]) }}" method="post">
                                                <i class="flaticon2-trash text-danger"></i></a>
                                        </td> --}}
                                        <td style="display: flex" >
                                            <form action="{{ route('admin.certificate.delete', ['id' => $db->id])}}" method="post">
                                                @csrf
                                                @method('DELETE')
