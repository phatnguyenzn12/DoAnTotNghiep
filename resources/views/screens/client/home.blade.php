...
@section('content')
    ...
    @foreach ($interView as $item)
        <li>
            <div class="uk-position-cover bg-inherit" uk-slideshow-parallax="scale: 1.2,1.2,1">
                <img src="{{asset($item->image)}}" class="object-cover " alt="" uk-cover>
            </div>
            <div class="container relative md:p-20 md:mt-7 p-5 h-full">
                <div uk-slideshow-parallax="scale: 1,1,0.8"
                    class="flex flex-col justify-center h-full w-full space-y-3">
                    <h1 uk-slideshow-parallax="y: 100,0,0" class="lg:text-4xl text-2xl text-white font-semibold"> {{$item->title}}</h1>
                    <p uk-slideshow-parallax="y: 150,0,0" class="text-base text-white font-medium pb-4 lg:w-1/2"> {{$item->content}} </p>
