            ...
            <div class="col-12">
                ...
                <div class="tiny-slider arrow-round arrow-blur arrow-hover rounded-3 overflow-hidden">
                    <div class="tiny-slider-inner" data-autoplay="false" data-gutter="0" data-arrow="true" data-dots="false"
                        data-items="1">
                        <!-- Card item START -->
                        @foreach($interView as $banner)
                            <div class="card overflow-hidden h-500px h-md-600px text-center rounded-0"
                                style="background-image:url({{asset('app/'.$banner->image)}}); background-position: center left; background-size: cover;">
                                    ...
                                    <div class="w-100 my-auto">
                                        <div class="row justify-content-center">
                                            <div class="col-11 col-lg-6">
                                                <!-- Title -->
                                                <h1 class="mb-0 text-white display-6">{{$banner->title}}
                                                        <!-- SVG START -->
                                                        <span
                                                ...
                                                <h3 class="text-danger " style="font-style: italic">{{$banner->discountCode->discount}}%</h3>
                                                <h4 class="text-white">Mã giảm giá: {{$banner->discountCode->code}}</h4>
                                                <p class="text-white">{{$banner->content}}</p>
                                                <!-- Button -->
                                                <a href="{{ route('client.course.show', ['slug' => $banner->course->slug, 'course' => $banner->course_id]) }}" class="btn btn-white mb-0">Get Started</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
